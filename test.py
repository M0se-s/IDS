...
def test_ids():
    ...
    # collect a small set of normal traffic features for training before we
    # start detection.  in real usage this would come from observed benign
    # traffic over time.
    normal_features = []
    for packet in test_packets[:2]:  # first two packets are normal
        feats = ids.traffic_analyzer.analyze_packet(packet)
        if feats:
